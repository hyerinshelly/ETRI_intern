...
- 코로나 추이 예측(배팅)에 필요한 환경 구성을 완료하였고 baseline 라이브러리의 ACKTR을 사용해 학습해보았다. 라이브러리의 모델로는 학습에 사용한 데이터 이외에는 예측이 불가능하여, 『강화학습 입문 파이썬 예제와 함께하는』, 『수학으로 풀어보는 강화학습 원리와 알고리즘』책을 참고하여 환경에 맞게 모델을 만들고 학습시켜 봐야겠다. (move37의 value iteration 수정)
