...
- 주간 계획
    ...
    - 『수학으로 풀어보는 강화학습 원리와 알고리즘』 4장 A2C, 5장 A3C, 6장 PPO, 7장 DDPG 공부 - [github 링크](https://github.com/kjwoo31/Reinforcement-Learning-Book)
...
- 코로나 추이 예측 환경 구성
    ...
    - 문제점
        - 라이브러리의 모델로는 학습에 사용한 데이터 이외에는 예측이 불가능하여, 책을 참고하여 환경에 맞게 모델을 만들고 학습해야 한다.
        - 다시 모델을 만들어 학습한다고 하더라도 데이터 사이의 연관성이 명확하지 않아 현재 구성한 환경으로는 데이터를 넣었을 때 추이 예측이 불가능해보인다.
