...
- 코로나 추이 예측(배팅)에 필요한 환경 구성을 완료하였고 baseline 라이브러리의 ACKTR을 사용해 학습해보았다. 라이브러리의 모델로는 학습에 사용한 데이터 이외에는 예측이 불가능하여, 책을 참고하여 환경에 맞게 모델을 만들고 학습해야 한다.
- 그런데, 데이터 사이의 연관성이 명확하지 않아 현재 구성한 환경으로는 데이터를 넣었을 때 추이 예측이 불가능해보인다. 그래서 코로나 관련 지수를 변수로 갖는 일차함수를 만들고, 최적의 상수를 찾는 강화학습 방법을 대신 구현해야 겠다.
